...
------------------------------------------------------------------------- Object-Tag Relations (N-N relation between tags and cubeobjects)
...
BEGIN
  SELECT tagset_id INTO tagset_id_for_hierarchy
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM public.tags t
    WHERE t.id = NEW.tag_id AND t.tagset_id = tagset_id_for_hierarchy
  ) THEN
