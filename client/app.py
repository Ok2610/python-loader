...
@click.option('--attributes', '-a', multiple=True, default=['id', 'file_uri', 'file_type', 'thumbnail_uri'],
	      					help='Attributes to include (default= id, file_uri, file_type, thumbnail_uri)')
@click.pass_context
def get(ctx, id, attributes):
	"""Get a single cubeobject with the given ID"""
	if (id) and (id>0):
		if ctx.obj['DEBUG']:
			click.echo(f"DEBUG: Getting cubeobject with ID: {id}")
...
@click.pass_context
def get_id(ctx, file_uri):
	"""Get a an object ID using its URI"""
	if ctx.obj['DEBUG']:
		click.echo(f"DEBUG: Getting cubeobject with ID: {id}")
	with grpc.insecure_channel(SERVER_ADDRESS) as channel:
		stub =  medias_pb2_grpc.MediasStub(channel)
		request = medias_pb2.GetMediaIdFromURIRequest(uri=file_uri)
		response = stub.GetMediaIdFromURI(request)
		if response.success:
			click.echo(response.id)
		else:
			click.echo("Error: URI not found in database.")
...
@click.option('--attributes', '-a', multiple=True, default=['id', 'file_uri', 'file_type', 'thumbnail_uri'],
	      					help='Attributes to include (default= id, file_uri, file_type, thumbnail_uri)')
def listall(ctx, attributes):
		"""List all the cubeobjects stored"""
		if ctx.obj['DEBUG']:
			click.echo("DEBUG: Getting all cubeobjects")
		with grpc.insecure_channel(SERVER_ADDRESS) as channel:
			stub =  medias_pb2_grpc.MediasStub(channel)
			request = medias_pb2.EmptyRequest()
...
def add(ctx, path, formats):
		"""Add file or files from a specified directory to the database."""
		if ctx.obj['DEBUG']:
			click.echo("DEBUG: Adding files from directory.")
		if os.path.isdir(path):
			add_dir(path, formats)
		elif os.path.isfile(path):
			add_file(path)
		else:
			click.echo("Error: invalid path provided.")

def add_dir(directory, formats):
	file_count = 0
	...
	with grpc.insecure_channel(SERVER_ADDRESS) as channel:
		...
		click.echo("Found %d files to add in the directory." % file_count)
		for response in response_iterator:
			if response.success:
				click.echo("[SERVER] Success, added %d cubeobjects to database." % (response.count))
			else:
				click.echo("[SERVER] Error, failed to add 1 packet of cubeobjects to database.")

def add_file(path):
	'''Probably not the best way of doing it but its an edge case and it works'''
	def add_single_media_request_generator (path):
	...
	with grpc.insecure_channel(SERVER_ADDRESS) as channel:
		stub =  medias_pb2_grpc.MediasStub(channel)
		response_iterator = stub.AddMedia(add_single_media_request_generator(path))
		click.echo("Adding 1 file to the DB...")
		for response in response_iterator:
			if response.success:
				click.echo("[SERVER] Success, added %d cubeobjects to database." % (response.count))
			else:
				click.echo("[SERVER] Error, failed to add cubeobject to database.")

@cli.command()
@click.argument('id', type=int)
@click.pass_context
def delete(ctx, id):
	"""Delete a single cubeobject with the given ID"""
	if (id) and (id>0):
		if ctx.obj['DEBUG']:
			click.echo(f"DEBUG: Deleting cubeobject with ID: {id}")
		with grpc.insecure_channel(SERVER_ADDRESS) as channel:
			stub =  medias_pb2_grpc.MediasStub(channel)
			request = medias_pb2.DeleteMediaRequest(id=id)
			response = stub.DeleteMedia(request)
			if response.success:
				click.echo("Object successfully removed from database.")
			else:
				click.echo("Error: index not found in database.")
	else:
		click.echo("ERROR: index must be > 0")
...
def reset(ctx):
	"""Reset the database"""
	if click.confirm("Do you want to reset the database ? All data will be lost.", default=False):
		if ctx.obj['DEBUG']:
		...
		with grpc.insecure_channel(SERVER_ADDRESS) as channel:
			...
			if response.success:
				...
				click.echo("Error: couldn't reset database.")
	else:
...
# 	print("%s, added %d cubeobjects to database" % (response.message, response.count))
